					 ...
					 off_t page_offset, size_t chunk, uint8_t *data)
{
	...
	if (ret == -EIO) {
		LOG_ERR("Uncorrectable ECC error detected, page=%d block=%d plane=%d", address->page, address->block, address->plane);
		...
		LOG_WRN("Rewrite recommended, page=%d block=%d plane=%d", address->page, address->block, address->plane);
	}
