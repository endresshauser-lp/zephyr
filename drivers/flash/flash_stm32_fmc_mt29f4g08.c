...
#ifdef CONFIG_FLASH_MT29F4G08_BAD_BLOCK_CACHE
	/* Bad block table */
	uint8_t *bad_block_table;
#endif /* CONFIG_FLASH_MT29F4G08_BAD_BLOCK_CACHE */
	uint8_t bank;
	size_t page_size;
...
static int flash_mt29f4g08_mark_bad_block(const struct device *dev, const off_t *offset)
{
	...
	spare_area[0] = 0x00;
	ret = flash_stm32_fmc_nand_write_spare_area(config->controller, &address, spare_area);
	...
	uint32_t bbt_idx = *offset / config->block_size;
...
#ifdef CONFIG_FLASH_MT29F4G08_BAD_BLOCK_CACHE
static int bad_block_table_is_bad_block(const struct device *dev, const off_t *offset,
					enum flash_block_status *status)
{
	const struct flash_mt29f4g08_config *config = dev->config;
	uint32_t bbt_idx;
	...
	bbt_idx = *offset / config->block_size;
...
static int build_bad_block_table(const struct device *dev)
{
	...
	/* Scan all blocks */
	for (off_t addr = 0; addr < config->flash_size; addr += config->block_size) {
		ret = flash_mt29f4g08_is_bad_block(dev, &addr, &status);
		if (ret != 0) {
			return ret;
		}
...
#ifdef CONFIG_FLASH_MT29F4G08_BAD_BLOCK_CACHE
		return bad_block_table_is_bad_block(dev, (const off_t *)in,
...
static int flash_stm32_fmc_mt29f4g08_init(const struct device *dev)
{
	...
	if (ret != 0) {
		LOG_ERR("Building bad block table failed with error %d", ret);
		return -EIO;
	}
#endif /* CONFIG_FLASH_MT29F4G08_BAD_BLOCK_CACHE */
...
#define FLASH_STM32_FMC_MT29F4G08_INIT(n)                                                          \
	IF_ENABLED(CONFIG_FLASH_MT29F4G08_BAD_BLOCK_CACHE, (                                       \
	static uint8_t flash_stm32_fmc_mt29f4g08_bad_block_table_##n[DIV_ROUND_UP(                 \
		DT_INST_PROP(n, flash_size) / DT_INST_PROP(n, block_size),                         \
		BITS_PER_BYTE)];                                                                   \
	))                                                                                         \
                                                                                                   \
	static const struct flash_mt29f4g08_config flash_stm32_fmc_mt29f4g08_config_##n = {        \
		...
		IF_ENABLED(CONFIG_FLASH_MT29F4G08_BAD_BLOCK_CACHE, (                               \
			.bad_block_table = flash_stm32_fmc_mt29f4g08_bad_block_table_##n,          \
		))                                                                                 \
		LAYOUT_PAGES_PROP(n)};                                                             \
                                                                                                   \
