...
#define DT_DRV_COMPAT st_stm32_xspi_psram_controller
...
LOG_MODULE_REGISTER(mspi_stm32_xspi_psram, CONFIG_MSPI_LOG_LEVEL);

struct mspi_stm_xspi_psram_config {
    struct mspi_stm32_conf mspi_cfg;
    uint8_t polling_cmd;
    uint8_t polling_mask;
};

static uint32_t mspi_stm32_xspi_psram_hal_address_size(uint8_t address_length)
{
 ...
 */
static XSPI_RegularCmdTypeDef mspi_stm32_xspi_psram_prepare_cmd(uint8_t cfg_mode, uint8_t cfg_rate)
{
	...
	cmd_tmp.InstructionDTRMode = (cfg_rate != MSPI_DATA_RATE_SINGLE)
					     ? HAL_XSPI_INSTRUCTION_DTR_ENABLE
					     : HAL_XSPI_INSTRUCTION_DTR_DISABLE;
	cmd_tmp.AlternateBytesMode = HAL_XSPI_ALT_BYTES_NONE;
	cmd_tmp.AddressDTRMode = (cfg_rate != MSPI_DATA_RATE_SINGLE) ? HAL_XSPI_ADDRESS_DTR_ENABLE
 ...
 */
static bool mspi_stm32_xspi_psram_is_memorymap(const struct device *dev)
{
 ...
 */
static int mspi_stm32_xspi_psram_memmap_off(const struct device *controller)
{
 ...
 */
static int mspi_stm32_xspi_psram_memmap_on(const struct device *controller)
{
	HAL_StatusTypeDef ret;
	struct mspi_stm32_data *dev_data = controller->data;
	XSPI_MemoryMappedTypeDef s_MemMappedCfg;

	if (mspi_stm32_xspi_psram_is_memorymap(controller)) {
		return 0;
	}

	/* Configure in MemoryMapped mode */
	if ((dev_data->dev_cfg.io_mode == MSPI_IO_MODE_SINGLE) &&
	    (mspi_stm32_xspi_psram_hal_address_size(dev_data->dev_cfg.addr_length) ==
	...
	XSPI_RegularCmdTypeDef s_command =
		mspi_stm32_xspi_psram_prepare_cmd(dev_data->dev_cfg.io_mode, dev_data->dev_cfg.data_rate);
	...
	s_command.AddressWidth = mspi_stm32_xspi_psram_hal_address_size(dev_data->dev_cfg.addr_length);
...
static int mspi_stm32_xspi_psram_context_lock(struct mspi_stm32_context *ctx,
...
static void mspi_stm32_xspi_psram_context_unlock(struct mspi_stm32_context *ctx)
{
	k_sem_give(&ctx->lock);
}
 ...
 */
static bool mspi_stm32_xspi_psram_is_inp(const struct device *controller)
{
	struct mspi_stm32_data *dev_data = controller->data;
...
static int mspi_stm32_xspi_psram_abort_memmap_if_enabled(const struct device *dev)
{
	int ret = 0;

	if (mspi_stm32_xspi_psram_is_memorymap(dev)) {
		ret = mspi_stm32_xspi_psram_memmap_off(dev);
		if (ret != 0) {
...
static int read_write_in_memory_map_mode(const struct device *dev, struct mspi_xfer_packet *packet)
{
	...
	if (!mspi_stm32_xspi_psram_is_memorymap(dev)) {
		ret = mspi_stm32_xspi_psram_memmap_on(dev);
		if (ret != 0) {
	...
	ret = mspi_stm32_xspi_psram_abort_memmap_if_enabled(dev);
	if (ret != 0) {
		return ret;
	}
 ...
 *
 */
static int mspi_stm32_xspi_psram_access(const struct device *dev, struct mspi_xfer_packet *packet,
				  uint8_t access_mode)
{
	HAL_StatusTypeDef hal_ret;
	struct mspi_stm32_data *dev_data = dev->data;
	...
	if (dev_data->xip_cfg.enable) {
		if ((packet->cmd == MSPI_NOR_CMD_WREN) || (packet->cmd == MSPI_NOR_OCMD_WREN) ||
			...
			int ret = mspi_stm32_xspi_psram_abort_memmap_if_enabled(dev);
...
indirect:
	...
	cmd.AddressWidth = mspi_stm32_xspi_psram_hal_address_size(dev_data->ctx.xfer.addr_length);
	if (cmd.DataLength == 0) {
...
static int mspi_stm32_xspi_psram_config_mem(const struct device *dev, uint8_t cfg_mode, uint8_t cfg_rate)
{
	struct mspi_stm32_data *dev_data = dev->data;
...
static void mspi_stm32_xspi_psram_isr(const struct device *dev)
{
	struct mspi_stm32_data *dev_data = dev->data;
...
static __maybe_unused void mspi_stm32_xspi_psram_dma_callback(const struct device *dev, void *arg,
...
static int mspi_stm32_xspi_psram_validate_freq(uint32_t freq, uint32_t max_freq)
{
	if (freq > max_freq) {
...
static int mspi_stm32_xspi_psram_validate_io_mode(uint32_t io_mode)
{
...
static int mspi_stm32_xspi_psram_validate_data_rate(uint32_t data_rate)
{
...
static int mspi_stm32_xspi_psram_validate_cpp(uint32_t cpp)
{
...
static int mspi_stm32_xspi_psram_validate_endian(uint32_t endian)
{
...
static int mspi_stm32_xspi_psram_validate_ce_polarity(uint32_t ce_polarity)
{
...
static int mspi_stm32_xspi_psram_validate_dqs(bool dqs_enable, bool dqs_support)
{
				 ...
				 const struct mspi_stm32_conf *cfg)
{
	...
	if ((param_mask & MSPI_DEVICE_CONFIG_CE_POL) != 0) {
		ret = mspi_stm32_xspi_psram_validate_ce_polarity(dev_cfg->ce_polarity);
		if (ret != 0) {
			goto end;
		}
	...
	if ((param_mask & MSPI_DEVICE_CONFIG_DQS) != 0) {
		ret = mspi_stm32_xspi_psram_validate_dqs(dev_cfg->dqs_enable, cfg->mspicfg.dqs_support);
		if (ret != 0) {
			goto end;
		}
	...
	if ((param_mask & MSPI_DEVICE_CONFIG_ENDIAN) != 0) {
		ret = mspi_stm32_xspi_psram_validate_endian(dev_cfg->endian);
		if (ret != 0) {
			goto end;
		}
 ...
 */
static int mspi_stm32_xspi_psram_dev_cfg_save(const struct device *controller,
					const enum mspi_dev_cfg_mask param_mask,
					const struct mspi_dev_cfg *dev_cfg)
{
	...
	if ((param_mask & MSPI_DEVICE_CONFIG_FREQUENCY) != 0) {
		ret = mspi_stm32_xspi_psram_validate_freq(dev_cfg->freq, cfg->mspicfg.max_freq);
		if (ret != 0) {
			goto end;
		}
	...
	if ((param_mask & MSPI_DEVICE_CONFIG_IO_MODE) != 0) {
		ret = mspi_stm32_xspi_psram_validate_io_mode(dev_cfg->io_mode);
		if (ret != 0) {
			goto end;
		}
	...
	if ((param_mask & MSPI_DEVICE_CONFIG_DATA_RATE) != 0) {
		ret = mspi_stm32_xspi_psram_validate_data_rate(dev_cfg->data_rate);
		if (ret != 0) {
			goto end;
		}
	...
	if ((param_mask & MSPI_DEVICE_CONFIG_CPP) != 0) {
		ret = mspi_stm32_xspi_psram_validate_cpp(dev_cfg->cpp);
		if (ret != 0) {
			goto end;
		}
 ...
 */
static int mspi_stm32_xspi_psram_dev_config(const struct device *controller,
				      ...
				      const struct mspi_dev_cfg *dev_cfg)
{
	...
	if (mspi_stm32_xspi_psram_is_inp(controller)) {
		if (locked) {
	...
	if (param_mask & (MSPI_DEVICE_CONFIG_IO_MODE | MSPI_DEVICE_CONFIG_DATA_RATE)) {
		...
		ret = mspi_stm32_xspi_psram_config_mem(controller, dev_cfg->io_mode, dev_cfg->data_rate);
		if (ret != 0) {
			goto e_return;
		}
	...
	if (mspi_stm32_xspi_psram_dev_cfg_save(controller, param_mask, dev_cfg) != 0) {
 ...
 * @retval A negative errno value upon failure.
 */
static int mspi_stm32_xspi_psram_xip_config(const struct device *controller,
				      const struct mspi_dev_id *dev_id,
				      const struct mspi_xip_cfg *xip_cfg)
{
	...
	if (!xip_cfg->enable) {
		...
		ret = mspi_stm32_xspi_psram_memmap_off(controller);
	} else {
		ret = mspi_stm32_xspi_psram_memmap_on(controller);
	}
 ...
 * @retval A negative errno value upon failure.
 */
static int mspi_stm32_xspi_psram_get_channel_status(const struct device *controller, uint8_t ch)
{
	struct mspi_stm32_data *dev_data = controller->data;
	int ret = 0;
...
static int mspi_stm32_xspi_psram_pio_dma_transceive(const struct device *controller,
					      const struct mspi_xfer *xfer)
{
	int ret = 0;
	uint32_t packet_idx;
	...
	ret = mspi_stm32_xspi_psram_context_lock(ctx, xfer);
	if (ret != 0) {
	...
	while (ctx->packets_left > 0) {
		...
		if (dev_cfg->dma_specified) {
			ret = mspi_stm32_xspi_psram_access(controller, packet, MSPI_ACCESS_DMA);
		} else {
...
#else
		ret = mspi_stm32_xspi_psram_access(controller, packet,
	...
	mspi_stm32_xspi_psram_context_unlock(ctx);
	return ret;
}
 ...
 * @retval A negative errno value upon failure.
 */
static int mspi_stm32_xspi_psram_transceive(const struct device *controller,
				      const struct mspi_dev_id *dev_id,
				      const struct mspi_xfer *xfer)
{
	int ret = 0;
    ...
    ret = mspi_stm32_xspi_psram_pio_dma_transceive(controller, xfer);

	return ret;
}

static int mspi_stm32_xspi_psram_dma_init(DMA_HandleTypeDef *hdma, struct stm32_stream *dma_stream)
{
	int ret;
	/*
	 * DMA configuration
...
static int mspi_stm32_xspi_psram_activate(const struct device *dev)
{
	int ret;
			 ...
			 uint32_t ahb_clock_freq)
{
	...
	dev_data->hmspi.xspi.Init.WrapSize = HAL_XSPI_WRAP_32_BYTES;
					 ...
					 struct mspi_stm32_data *dev_data)
{
	...
	if (mspi_stm32_xspi_psram_dma_init(&dev_data->hdma_tx, &dev_data->dma_tx) != 0) {
		LOG_ERR("XSPI DMA Tx init failed");
		return -EIO;
	}
	__HAL_LINKDMA(&dev_data->hmspi.xspi, hdmatx, dev_data->hdma_tx);

	if (mspi_stm32_xspi_psram_dma_init(&dev_data->hdma_rx, &dev_data->dma_rx) != 0) {
 ...
 */
static int mspi_stm32_xspi_psram_config(const struct mspi_dt_spec *spec)
{
	int ret;
	...
	ret = mspi_stm32_xspi_psram_activate(spec->bus);
	if (ret != 0) {
		goto end;
	}
	...
	if (ret == 0) {
		...
		LOG_ERR("MSPI not configured: %d", ret);
    }
...
static int mspi_stm32_init(const struct device *controller)
{
	...
	return mspi_stm32_xspi_psram_config(&spec);
}

static DEVICE_API(mspi, mspi_stm32_driver_api) = {
	.config = mspi_stm32_xspi_psram_config,
	.dev_config = mspi_stm32_xspi_psram_dev_config,
	.xip_config = mspi_stm32_xspi_psram_xip_config,
	.get_channel_status = mspi_stm32_xspi_psram_get_channel_status,
	.transceive = mspi_stm32_xspi_psram_transceive,
};

#ifdef CONFIG_PM_DEVICE
static int mspi_stm32_xspi_psram_suspend(const struct device *dev)
{
	int ret;
...
static int mspi_stm32_xspi_psram_pm_action(const struct device *dev, enum pm_device_action action)
{
	switch (action) {
	case PM_DEVICE_ACTION_RESUME:
		return mspi_stm32_xspi_psram_activate(dev);
	case PM_DEVICE_ACTION_SUSPEND:
		return mspi_stm32_xspi_psram_suspend(dev);
	default:
		return -ENOTSUP;
	}
}
...
#define XSPI_DMA_CHANNEL_INIT(node, dir, dir_cap, src_dev, dest_dev)                              \
	...
	.cfg = {                                                                                  \
		...
		.dma_callback = mspi_stm32_xspi_psram_dma_callback,                                     \
	},                                                                                        \
	.src_addr_increment =                                                                     \
...
#define STM32_SMPI_IRQ_HANDLER(index)                                                             \
	...
	{                                                                                         \
		IRQ_CONNECT(DT_INST_IRQN(index), DT_INST_IRQ(index, priority),                    \
			    mspi_stm32_xspi_psram_isr, DEVICE_DT_INST_GET(index), 0);                   \
	...
	static struct mspi_stm32_data mspi_stm32_dev_data_##index = {                             \
		.hmspi.xspi = {                                                                   \
			...
			.Init = {                                                                 \
				...
				.ChipSelectBoundary = 5,                                          \
				.MemoryMode = HAL_XSPI_SINGLE_MEM,                                \
				.MemorySize = DT_INST_PROP(index, addr_bits),                     \
		...
		XSPI_DMA_CHANNEL(DT_DRV_INST(index), rx, RX, PERIPHERAL, MEMORY)                  \
                                                                                                  ...
                                                                                                  \
	PM_DEVICE_DT_INST_DEFINE(index, mspi_stm32_xspi_psram_pm_action);                               \
